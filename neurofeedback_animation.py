...
# ------------------ Neurofeedback Functions ------------------

def compute_tbr_multitaper(sig, sfreq):
    sig = sig[np.newaxis, :]
    psd, freqs = mne.time_frequency.psd_array_multitaper(sig, sfreq=sfreq,
                                                        fmin=theta_band[0],
                                                        fmax=beta_band[1],
                                                        verbose=False)
    theta_idx = (freqs >= theta_band[0]) & (freqs <= theta_band[1])
    beta_idx = (freqs >= beta_band[0]) & (freqs <= beta_band[1])
    theta_power = np.mean(psd[0, theta_idx]) if np.any(theta_idx) else 0
    beta_power = np.mean(psd[0, beta_idx]) if np.any(beta_idx) else 1e-9
    return theta_power / (beta_power if beta_power > 1e-9 else 1e-9)
...
class FlowerNeurofeedback(tk.Frame):
    def __init__(self, master):
        ...
        self.saved_tbrs = []     # will store [timestamp, avg_tbr]
        self.channel_names = []
        ...
        self.tbr_value_label = tk.Label(bottom_frame, text="TBR: N/A", font=("Arial", 16), bg="#E0F7FA")
        self.tbr_value_label.pack(pady=5)
    ...
    def _setup_tbr_plot(self, tbr_win):
        """Sets up the TBR plot within the Toplevel window."""
        fig, ax = plt.subplots(facecolor='#E0F7FA', figsize=(6, 4))
        ax.set_title("Real-time Average TBR")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Average TBR")
        ax.set_xlim(0, 300) # Initial time limit
        ax.set_ylim(0, 5) # Initial TBR limit
        ax.grid(True)
        # Add horizontal lines for thresholds
        ax.axhline(y=1.0, color='r', linestyle='--', label='TBR = 1.0')
        ax.axhline(y=1.5, color='orange', linestyle=':', label='TBR = 1.5')
    ...
    def _setup_tbr_window(self):
        ...
        # --- 1. Create the Toplevel window for the TBR plot ---
        self.tbr_window = tk.Toplevel(self.master)
        self.tbr_window.title("Real-time TBR Plot")
        ...
        self.tbr_data_x.clear()
        self.tbr_data_y.clear()
    ...
    def update_tbr_plot(self, avg_tbr):
        ...
        # 1. Update data lists
        self.tbr_data_x.append(current_time)
        self.tbr_data_y.append(avg_tbr)
        
        # 2. Update plot line data
        self.tbr_line.set_data(self.tbr_data_x, self.tbr_data_y)
        ...
        y_max = max(5, np.max(self.tbr_data_y) * 1.1)
        self.tbr_ax.set_ylim(0, y_max)
        
        # 5. Redraw the canvas
        self.tbr_canvas.draw_idle()

        # 6. Update the TBR label on the main screen
        self.tbr_value_label.config(text=f"TBR: {avg_tbr:.2f}")
    ...
    def run_neurofeedback(self):
        ...
        while self.remaining_time>0:
            ...
            if ptr%samples_in_buffer==0:
                raw_buffer_np = np.array(data_buffer).T
                ica_sources = np.dot(raw_buffer_np, unmixing_matrix.T)
                clean_data = np.dot(ica_sources, mixing_matrix_clean.T)
                clean_occ_data = clean_data[:, occ_indices]

                tbrs = []
                for i in range(clean_occ_data.shape[1]):
                    tbr = compute_tbr_multitaper(clean_occ_data[:, i], fs)
                    tbrs.append(tbr)
                avg_tbr = np.mean(tbrs)

                current_time = 300 - self.remaining_time
                self.saved_tbrs.append([current_time, avg_tbr])


                # --- Safely update the TBR plot in the separate window ---
                # This must be scheduled on the main thread via self.master.after()
                self.master.after(0, self.update_tbr_plot, avg_tbr)
                # ---------------------------------------------------------

                # ---------------- Flower logic based on TBR ----------------
                if avg_tbr < 1.0:
    ...
    def display_result(self):
        ...
        self.save_results_numeric(f"results/numeric_results_sub{subject}_session{session}_run{run}.txt")
    ...
    def save_as_fif(self):
        ...
        # Save the file
        out_file = f"results/eeg_data_sub{subject}_session{session}_run{run}.fif"
    ...
    def save_results(self):
        ...
        # --- Save EEG ---
        eeg_file = f"results/eeg_data_sub{subject}_session{session}_run{run}.csv"
        ...
        # --- Save TBR ---
        tbr_file = f"results/tbr_values_sub{subject}_session{session}_run{run}.csv"
        with open(tbr_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["time_elapsed_sec", "avg_tbr"])
            writer.writerows(self.saved_tbrs)

        print(f"Saved EEG to {eeg_file} and TBR to {tbr_file}")
    ...
    def restart_test(self):
        ...
        # Reset TBR display
        self.tbr_value_label.config(text="TBR: N/A")
